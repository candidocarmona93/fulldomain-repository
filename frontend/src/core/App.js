...
export class App extends BaseWidget {
  ...
  routes(routePath, component, callback = null) {
    ...
    if (this.routesCallback[path]) {
      console.warn(`Duplicate route callback for path: ${path}`);
    }

    this.routesCallback[path] = callback;
      ...
      for (const [routePattern, routeDefinition] of routes) {
        try {
          ...
          if (match) {
            ...
            const decodedPaths = {};
            match.inputs.forEach(input => {
              for (const [key, value] of Object.entries(input || {})) {
                decodedPaths[key] = decodeURIComponent(value);
              }
            })

            return {
              component: routeDefinition,
              params: { ...decodedParams, ...decodedPaths },
  ...
  render() {
    ...
    for (const [routePattern, routeDefinition] of routes) {
      try {
        ...
        if (match) {
          const callback = this.routesCallback[routePattern];
          if (callback) {
            callback(match.pathname.groups);
          }
        }
      } catch (error) {
        console.warn(`Skipping invalid route pattern: ${routePattern}`, error);
  ...
  renderRoute() {
    if (!this.rootElement) {
      throw new Error("Invalid root element or reference.");
    }
    ...
    try {
      ...
      if (matchedRouteDefinition) {
        ...
        finalComponentToRender = new PageComponentClass({ args: { ...this.params }, });
        const layouts = [...matchedRouteDefinition.layouts];

        while (layouts.length > 0) {
          let deepComponentLayout = layouts.pop();
          let layoutInstance = new deepComponentLayout();
